...
func TestRemoveDirectoryWithContents(t *testing.T) {
	...
	errmkdir := os.MkdirAll(filepath.Join(testbasedir, "directory", "structure"), 0700)
	if errmkdir != nil {
	...
	errwrt := ioutil.WriteFile(filepath.Join(testbasedir, "testfile.txt"),  b, 0600)
	if errwrt != nil {

		t.Fatalf("%s failed.  Writing internal test file base dir.", t.Name())
	}

	errwrt = ioutil.WriteFile(filepath.Join(testbasedir, "directory", "testfile.txt"),  b, 0600)
	if errwrt != nil {

		t.Fatalf("%s failed.  Writing internal test file directory dir.", t.Name())
	}

	errwrt = ioutil.WriteFile(filepath.Join(testbasedir, "directory", "structure", "testfile.txt"),  b, 0600)
	if errwrt != nil {
